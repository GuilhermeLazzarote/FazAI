...
(function (global) {
  function _resolveBase() {
    if (typeof FAZAI_BASE !== 'undefined' && FAZAI_BASE) return FAZAI_BASE;
    var g = (typeof window !== 'undefined' ? window : globalThis);
    if (g && g.FAZAI_BASE) return g.FAZAI_BASE;
    if (typeof require !== 'undefined') { try { return require('./fazai_base_dados').FAZAI_BASE; } catch(e){} }
    return null;
  }
  function B_() { return _resolveBase(); }
  var B = _resolveBase();
  ...
  function indiceCorrecaoIPCAE(comp, dataRef, avisos) {
    const a = lookupSerie(B_().ipcae, comp), b = lookupSerie(B_().ipcae, dataRef);
  ...
  function fatorSelic(dataDistrib, dataAtual, avisos) {
    const a = lookupSerie(B_().selic, dataDistrib), b = lookupSerie(B_().selic, dataAtual);
  ...
  let _sm = null; function seriaSM() { if (!_sm) { _sm = {}; B_().salarioMinimo.forEach(x => _sm[x.vigencia] = x.valor); } return _sm; }
  ...
  function agregar(resultados, caso) {
    ...
    return {
      ...
      _meta: { base: (B_() && B_()._meta) || null, avisos: [...new Set([...avisos, ...verbas.flatMap(v => v.avisos || [])])] },
    };
  }
  ...
  function tabelaInssDoMes(comp) {
    const chave = ym(comp); const chaves = Object.keys(B_().inss).sort();
    let esc = null; for (const c of chaves) { if (c <= chave) esc = c; else break; }
    return esc ? B_().inss[esc] : null;
  }
  ...
  // ───────────────────────── Família E — Verbas rescisórias ─────────────────────────
  // Calculadas no fim do contrato (verba de bloco/competência da rescisão), exatas e auditáveis.
  // Usam a remuneração do último mês (caso.salariosMensais) e as datas. Reflexos de habituais
  // (HE, adicionais) podem ser somados via caso.mediaReflexosRescisao (opcional).
  function _remuneracaoFinal(caso) {
    const sm = caso.salariosMensais || {};
    const chaves = Object.keys(sm).sort();
    return chaves.length ? sm[chaves[chaves.length - 1]] : 0;
  }
  function _anosCompletos(caso) { return Math.floor(mesesEntre(caso.dataAdmissao, caso.dataDemissao) / 12); }
  function _diasAviso(caso) { return Math.min(30 + _anosCompletos(caso) * 3, 90); }

  function calcularRescisorias(caso, quais) {
    const avisos = [];
    const rem = _remuneracaoFinal(caso);
    const compResc = ym(caso.dataDemissao);
    const indice = indiceCorrecaoIPCAE(caso.dataDemissao, caso.dataDistribuicao, avisos);
    const semJusta = caso.motivoDemissao === 'Sem Justa Causa';
    const linhas = [];
    const add = (descricao, valor, natureza, metodologia) => {
      if (valor > 0) linhas.push({ id: descricao, descricao, natureza, metodologia,
        memoriaMensal: [{ competencia: compResc, valorDevido: round(valor), totalApurado: round(valor),
          indiceCorrecao: round(indice, 6), corrigido: round(valor * indice) }],
        principal: round(valor), fgts: 0, valorCorrigido: round(valor * indice),
        origem: 'motor', formato: 'mensal', _tipo: 'rescisoria', avisos: [] });
    };

    const sel = quais || ['saldoSalario', 'avisoPrevio', 'decimoTerceiro', 'ferias', 'fgtsRescisorio', 'multaFgts'];
    // Saldo de salário: salário ÷ 30 × dias trabalhados no último mês
    const diasUltimoMes = caso.diasTrabalhadosUltimoMes != null ? caso.diasTrabalhadosUltimoMes : caso.dataDemissao.getDate();
    const saldo = rem / 30 * diasUltimoMes;
    // Aviso prévio indenizado (só sem justa causa)
    const aviso = semJusta ? _diasAviso(caso) / 30 * rem : 0;
    // 13º proporcional (meses no ano da rescisão; mês>=15 dias conta)
    const mesesAno = (caso.dataDemissao.getMonth() + 1) - (caso.dataDemissao.getFullYear() === caso.dataAdmissao.getFullYear() ? caso.dataAdmissao.getMonth() : 0);
    const decimo = rem / 12 * Math.max(0, mesesAno);
    // Férias proporcionais + 1/3 (período aquisitivo incompleto)
    const mesesContrato = mesesEntre(caso.dataAdmissao, caso.dataDemissao);
    const mesesAquisIncompleto = mesesContrato % 12;
    const feriasProp = (rem / 12 * mesesAquisIncompleto) * (4 / 3);
    // Férias vencidas (período completo não gozado), se informado
    const feriasVenc = (caso.feriasVencidas ? rem * (4 / 3) : 0);
    const ferias = feriasProp + feriasVenc;
    // FGTS rescisório (8% sobre saldo + aviso + 13º)
    const baseFgts = saldo + (semJusta ? aviso : 0) + decimo;
    const fgtsResc = baseFgts * 0.08;
    // Multa 40% (sobre o FGTS do contrato; usa caso.fgtsDepositadoContrato se houver, senão estima)
    const fgtsContrato = caso.fgtsDepositadoContrato != null ? caso.fgtsDepositadoContrato : (rem * 0.08 * mesesContrato);
    const multa40 = semJusta ? fgtsContrato * 0.40 : 0;

    if (sel.includes('saldoSalario')) add('SALDO DE SALÁRIO', saldo, 'salarial', `Salário ÷ 30 × ${diasUltimoMes} dias.`);
    if (sel.includes('avisoPrevio') && aviso > 0) add('AVISO PRÉVIO INDENIZADO', aviso, 'indenizatoria', `${_diasAviso(caso)} dias (Lei 12.506) ÷ 30 × remuneração.`);
    if (sel.includes('decimoTerceiro')) add('13º SALÁRIO PROPORCIONAL', decimo, 'salarial', `Remuneração ÷ 12 × ${Math.max(0, mesesAno)} meses.`);
    if (sel.includes('ferias')) add('FÉRIAS PROPORCIONAIS + 1/3', ferias, 'indenizatoria', `Remuneração ÷ 12 × ${mesesAquisIncompleto} meses + 1/3${feriasVenc ? ' (+ férias vencidas)' : ''}.`);
    if (sel.includes('fgtsRescisorio')) add('FGTS RESCISÓRIO (8%)', fgtsResc, 'fgts', `8% sobre saldo + aviso + 13º.`);
    if (sel.includes('multaFgts') && multa40 > 0) add('MULTA 40% FGTS', multa40, 'indenizatoria', `40% do FGTS do contrato.`);
    return linhas;
  }
