...
(function (global) {
  ...
  function calcularLiquidacaoDeterministica(caso) {
    ...
    (caso.verbasAtivas || []).forEach(id => {
      ...
      const cfg = { ...r.cfg, ...ov };
      if (r.tipo === 'adicional') resultados.push(M.calcularAdicional(cfg, caso));
      else if (r.tipo === 'he') resultados.push(M.calcularHE(cfg, caso));
      else if (r.tipo === 'auxilio') {
        cfg.valorMensalPadrao = (caso.auxiliosPadrao && caso.auxiliosPadrao[id]) || 0;
