...
5) VERBAS ALTERNATIVAS / OPÇÃO: quando a decisão defere insalubridade E periculosidade mandando OPTAR pela mais favorável nos períodos concomitantes (não cumulam — art.193 §2º CLT, Tema 17 TST), a AUSÊNCIA de uma delas no cálculo NÃO é erro — é a opção exercida. O mesmo vale para qualquer verba deferida "em alternativa" a outra. Nesses casos, no máximo registre "conferir se a opção foi pela mais favorável", nunca "verba deferida não apurada".
6) SÓ conclua "NÃO APURADA" se, DEPOIS de casar por natureza, desdobramento, reflexo e opção, a verba deferida (que gera valor) realmente não tiver NENHUMA rubrica correspondente no cálculo. SÓ conclua "SEM DEFERIMENTO" se uma rubrica de valor não corresponder a NENHUMA verba deferida NEM for reflexo/dedução de uma. Antes de afirmar qualquer um dos dois, releia o mapeamento — esses dois erros normalmente são o MESMO item mal casado, contado dos dois lados.
════════════════════════════════════════════════════════════════
...
async function onCalcAdversario(input){
  ...
  try{
    ...
    // --- Detecção pelo CONTEÚDO (não só pela extensão): PDF começa com %PDF, XML com <?xml/<Calculo, zip com PK ---
    const bytes0=new Uint8Array(ab);
    const cab0=new TextDecoder('utf-8',{fatal:false}).decode(bytes0.slice(0,256));
    const ehPDF0 = cab0.startsWith('%PDF');
    const ehXML0 = /<\?xml|<Calculo|<calculo|<gprec/i.test(cab0);
    const ehZip0 = bytes0[0]===0x50 && bytes0[1]===0x4B;
    const ehPJC0 = !ehPDF0 && (ehXML0 || ehZip0 || nome.endsWith('.pjc') || nome.endsWith('.xml'));
    ...
    if(ehPJC0){
      ...
      // fallback: não reconheci a estrutura de rubricas → manda o conteúdo do XML como texto (a IA ainda lê)
      const txtxml=xml.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();
      if(txtxml.length>200){
        calcAdvTexto='CÁLCULO (conteúdo do arquivo do PJe-Calc, extraído como texto):\n'+txtxml.slice(0,40000);
        st.innerHTML='<span class="okmsg">✓ '+f.name+' lido (texto do PJe-Calc).</span> Clique no botão para gerar.';
        document.getElementById('imp-gerar').disabled=false; input.value=''; return;
      }
      st.innerHTML='<span style="color:#a11">Li o .PJC mas não reconheci o conteúdo. Tente exportar o cálculo em PDF e subir o PDF.</span>'; input.value=''; return;
    }
    ...
    try{
      ...
      // PDF do PJe-Calc pode ter 100+ páginas. A conferência precisa de DUAS coisas:
      //  (1) o RESUMO das verbas (primeiras páginas), e
      //  (2) AMOSTRAS do cartão de ponto / apuração de jornada, que ficam espalhadas
      //      ao longo do PDF por período (jornadas diferentes = trechos diferentes).
      // Então: lê o começo + páginas de amostra distribuídas pelo resto do PDF.
      const total=pdf.numPages;
      const paginasLer=new Set();
      // (1) resumo: primeiras 12 páginas
      for(let i=1;i<=Math.min(12,total);i++) paginasLer.add(i);
      // (2) amostras do resto: ~10 páginas distribuídas uniformemente (pega vários períodos de ponto)
      if(total>12){
        const nAmostras=Math.min(10, total-12);
        const passo=Math.max(1, Math.floor((total-12)/nAmostras));
        for(let i=13;i<=total && paginasLer.size<22;i+=passo) paginasLer.add(i);
        paginasLer.add(total); // última página costuma ter totais/fórmulas
      }
      const ordenadas=[...paginasLer].sort((a,b)=>a-b);
      let ultima=0;
      for(const i of ordenadas){
        if(i>ultima+1) texto+='\n[...páginas '+(ultima+1)+' a '+(i-1)+' omitidas (memória de cálculo repetitiva)...]\n';
        const pg=await pdf.getPage(i); const c=await pg.getTextContent();
        texto+='\n=== PÁGINA '+i+' de '+total+' ===\n'+c.items.map(x=>x.str).join(' ')+'\n';
        ultima=i;
        if(texto.length>45000) break; // teto de segurança
      }
      if(total>ordenadas.length) texto+='\n[NOTA: cálculo com '+total+' páginas. Foram lidas o resumo (início) + '+(ordenadas.length-12>0?ordenadas.length-12:0)+' amostras distribuídas do cartão de ponto/apuração, para conferir a jornada nos diferentes períodos sem processar todas as páginas. Se precisar conferir um período específico não amostrado, informe.]';
    }catch(e){ texto=''; }
...
async function gerarImpugnacao(){
  ...
  try{
    ...
    // chamada com RETRY: rede oscila e o provedor de IA às vezes responde "overloaded" (529) — tenta até 3x
    let res=null, data=null, ultimoErro='';
    for(let tent=1; tent<=3; tent++){
      try{
        ...
        data=await res.json();
        const sobrecarga = res.status===529 || res.status===503 || /overload|rate limit|capacity/i.test(String(data&&data.error||''));
        if(res.ok) break;
        ultimoErro=(data&&data.error)||('HTTP '+res.status);
        if(!sobrecarga) break; // erro real, não adianta repetir
      }catch(e){ ultimoErro=e.message; }
      if(tent<3){ st.innerHTML='<span class="hint">Provedor de IA momentaneamente sobrecarregado — tentando de novo ('+(tent+1)+'/3)…</span>'; await new Promise(r=>setTimeout(r,2500*tent)); }
    }
    if(!res || !res.ok) throw new Error('O provedor de IA está instável no momento ('+ultimoErro+'). Tente novamente em 1 minuto.');
